...
export const VideosPhotos = ({videosphotos}: Props) => {
  ...
  return (
    <div className="w-full py-4 md:px-5">
      ...
      <div className="grid grid-cols-1 relative">
        <Image
          src={getImageURL(getFindImage(videosphotos?.media as any))}
          className="max-w-full w-full "
          imageClassName="w-full rounded-lg md:h-96"
        />
        {videosphotos?.media?.length > 10 && (
      ...
      <div className="grid grid-cols-3 md:gap-4 gap-2 md:mt-4 mt-2">
        {videosphotos?.media?.map((image: string, index: number) => (
          <Image
            key={index}
      ...
      <ImageSilder open={open} close={() => setOpen(false)} images={videosphotos?.media} />
    </div>
  )
}
