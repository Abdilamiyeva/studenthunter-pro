...
export const Phd = ({selectProgram, setSelectProgram}: Props) => {
  const {id} = useParams()
  const [search, setSearch] = useState('')
  const {data: {data: faculty} = {}} = useGetUniversityFacultyQuery({university_id: id as string, level: 'MBA'})
  ...
  return (
    <div>
      {[...new Set(faculty?.map((item: any) => item?.faculty))]?.length ? (
        ...
        <div>
          <h2 className="flex justify-center items-center text-blue-2 h-20">No Data</h2>
        </div>
      )}
